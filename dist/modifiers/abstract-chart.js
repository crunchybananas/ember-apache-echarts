import { transform, merge } from 'lodash-es';
import { registerDestructor } from '@ember/destroyable';
import Modifier from 'ember-modifier';
import { use, init } from 'echarts/core';
import { CanvasRenderer } from 'echarts/renderers';
import { TitleComponent, LegendComponent, GridComponent, TooltipComponent, DataZoomComponent, GraphicComponent } from 'echarts/components';
import { LineChart, BarChart, PieChart, TreeChart, GraphChart } from 'echarts/charts';
import onElementResize from '../utils/on-element-resize.js';
import getUniqueDatasetValues from '../utils/data/get-unique-dataset-values.js';
import computeInnerBox from '../utils/layout/compute-inner-box.js';
import computeMaxTextMetrics from '../utils/layout/compute-max-text-metrics.js';
import computeTextHeight from '../utils/layout/compute-text-height.js';
import computeTextMetrics from '../utils/layout/compute-text-metrics.js';
import layoutCells from '../utils/layout/layout-cells.js';
...
use([CanvasRenderer, TitleComponent, LegendComponent, GridComponent, TooltipComponent, DataZoomComponent, GraphicComponent, LineChart, BarChart, PieChart, TreeChart, GraphChart]);
...
const Z_OVERLAY = 100;
class AbstractChartModifier extends Modifier {
  chart;
  resizeObserver;
  get defaultStyles() {
    return {
  ...
  createContext(args, chart) {
    ...
    const styles = transform(Object.keys(this.defaultStyles), (styles, type) => styles[type] = {
      ...baseStyle,
      // @ts-expect-error: follow up on this
  ...
  addChartBox(context, config) {
    ...
    mergeAtPaths(config, this.generateBoxConfig({
      ...style,
      ...context.layout
    }));
    return {
  ...
  addLegend(context, config) {
    ...
    mergeAtPaths(config, [this.generateLegendConfig(context.data.series, context.args, context.layout, style)]);
  ...
  addDataZoom(context, config) {
    ...
    if (xAxisZoom) {
      ...
      const xAxisZoomHeight = sliderHeight + brushSelectHeight + xAxisZoomStyle.marginTop + xAxisZoomStyle.marginBottom;
    ...
    if (yAxisZoom) {
      ...
      const yAxisZoomWidth = sliderWidth + brushSelectWidth + yAxisZoomStyle.marginLeft + yAxisZoomStyle.marginRight;
  ...
  addCellTitles(context, config) {
    ...
    mergeAtPaths(config,
    // @ts-expect-error: follow up on this
    layoutCells(context, context.data.series, (info, cell) => this.generateTitleConfig(info.label ?? info.name, {
      ...context.layout,
      width: cell.width,
      height: cell.height,
      x: cell.innerX,
  ...
  getLegendOrientation(args) {
    ...
    // @ts-expect-error: follow up on this
    return !['horizontal', 'vertical'].includes(legendOrientation ?? '') ? legend?.startsWith('top') || legend?.startsWith('bottom') ? 'horizontal' : 'vertical' : legendOrientation;
  }
    ...
    return {
      'graphic.elements': [{
        ...
        shape: {
          ...
          width: box.width - box.marginLeft - box.marginRight - box.borderLeftWidth - box.borderRightWidth,
          height: box.height - box.marginTop - box.marginBottom - box.borderTopWidth - box.borderBottomWidth
        },
        style: {
  ...
  generateLegendConfig(series, args, layout, style) {
    ...
    if (legend.startsWith('top') || legend.endsWith('Top')) {
      ...
      yLayout = {
        bottom: layout.chartHeight - layout.height - layout.y + 1 +
        // @ts-expect-error: Operator '+' cannot be applied to types 'number' and 'string | number'.
        style.marginBottom +
        // @ts-expect-error: Operator '+' cannot be applied to types 'number' and 'string | number'.
    ...
    if (legend.startsWith('left') || legend.endsWith('Left')) {
      ...
      //       has a border, it overlaps the chart border. [twl 2.Nov.22]
      xLayout = {
        right: layout.chartWidth - layout.width - layout.x + 1 +
        // @ts-expect-error: Operator '+' cannot be applied to types 'number' and 'string | number'.
        style.marginRight +
        // @ts-expect-error: Operator '+' cannot be applied to types 'number' and 'string | number'.
  ...
  generateXAxisDataZoomConfig(args, layout, style) {
    ...
    if (xAxisZoom === 'top') {
      ...
      config.bottom = layout.chartHeight - layout.height - layout.y + brushSelectHeight +
      // @ts-expect-error: Operator '+' cannot be applied to types 'number' and 'string | number'.
      style.marginBottom +
      // @ts-expect-error: Operator '+' cannot be applied to types 'number' and 'string | number'.
      style.borderBottomWidth / 2;
  ...
  generateYAxisDataZoomConfig(args, layout, style) {
    ...
    if (yAxisZoom === 'left') {
      ...
      config.right = layout.chartWidth - layout.width - layout.x + brushSelectWidth +
      // @ts-expect-error: Operator '+' cannot be applied to types 'number' and 'string | number'.
      style.marginRight +
      // @ts-expect-error: Operator '+' cannot be applied to types 'number' and 'string | number'.
      style.borderRightWidth / 2;
  ...
  computeLegendMetrics(context, config, style) {
    ...
    if (orientation === 'horizontal') {
      const labelMetrics = labels.reduce((result, label) => {
        // @ts-expect-error: follow up on this
      ...
      metrics.width = Math.min(layout.width, metrics.width + labelMetrics.width + itemGap * (labels.length - 1));
      ...
      metrics.height = metrics.height + labelMetrics.height * labels.length + itemGap * (labels.length - 1);
    }
    return metrics;
  }
}

export { AbstractChartModifier as default };
//# sourceMappingURL=abstract-chart.js.map
