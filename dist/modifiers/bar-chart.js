...
import { countBy, compact, flatten } from 'lodash-es';
import * as echarts from 'echarts/core';
import mergeAtPaths from '../utils/merge-at-paths.js';
import parseAxisLabel from '../utils/chart/parse-axis-label.js';
import computeStatistic from '../utils/data/compute-statistic.js';
import getSeriesData from '../utils/data/get-series-data.js';
import getSeriesTotals from '../utils/data/get-series-totals.js';
import getUniqueDatasetValues from '../utils/data/get-unique-dataset-values.js';
import rotateDataSeries from '../utils/data/rotate-data-series.js';
import computeMaxTextMetrics from '../utils/layout/compute-max-text-metrics.js';
...
const setItemColor = (colorMap, item, color) => !colorMap?.[color] ? item : {
  ...item,
  itemStyle: {
    color: colorMap[color]
  }
};
const isShowingAxisLabel = (axisConfig, labelType) => axisConfig.axisLabel?.[`show${labelType}Label`] === false || axisConfig.type === 'time' && axisConfig.axisLabel?.[`show${labelType}Label`] !== true;
// @ts-expect-error: follow up on this
const computeData = (data, categories, args) => {
  const {
    categoryProperty = DEFAULT_CATEGORY_PROPERTY
  } = args;
  const {
    categoryAxisType,
    orientation
  ...
  return categoryAxisType !== 'time' ? series :
  // @ts-expect-error: follow up on this
  series.map(item => ({
    ...item,
 ...
 */
class BarChartModifier extends AbstractChartModifier {
  static {
  ...
  formatTooltipParams(args, params, elementType) {
    const {
      valueAxisFormatter = echarts.format.addCommas
    } = args;
    const {
      categoryAxisType,
    ...
    return {
      ...params,
      value: !params.value && missingValueFormat != null ? missingValueFormat : valueAxisFormatter ? valueAxisFormatter(params.value, elementType) : params.value,
      category: !params.name && missingCategoryFormat != null ? missingCategoryFormat : categoryAxisFormatter ? categoryAxisFormatter(params.name, elementType) : params.name
    };
  }
  // @ts-expect-error: follow up on this
  configureChart(args, chart) {
    const allSeries = args.series ?? [{
      data: args.data
    }];
    const {
      categoryAxisScale,
    ...
    chart.setOption({
      ...
      tooltip: {
        trigger: 'item',
        ...(tooltipFormatter && {
          formatter: params => tooltipFormatter(params.length != null ? params.map(param => this.formatTooltipParams(args, param, 'axisTooltip')) : this.formatTooltipParams(args, params, 'itemTooltip'),
          // @ts-expect-error: follow up on this
    ...
    chart.on('selectchanged', event => {
      ...
      const name = categoryAxisScale === 'shared' ?
      // @ts-expect-error: follow up on this
      context.data.categories[dataIndex] :
      // @ts-expect-error: follow up on this
      series.data[dataIndex] ?
      // @ts-expect-error: follow up on this
      series.data[dataIndex][args.categoryProperty ?? DEFAULT_CATEGORY_PROPERTY] : null;
      if (name) {
  ...
  createContextData(args) {
    ...
    const seriesData = rotateData ? rotateDataSeries(context.series, categoryProperty, valueProperty) : context.series;
    const {
      series,
      title
    ...
    return {
      ...
      series: this.isStackedVariant(args.variant) || this.isGroupedVariant(args.variant) ? [{
        data: series
      }] : series,
      dataset: series,
      title
  ...
  computeCategoryInfo(series, context) {
    ...
    const seriesData = this.isGroupedVariant(variant) || this.isStackedVariant(variant) ? series.data : [series];
    const categories = categoryAxisScale === 'shared' ? data.categories : this.getCategories(args, seriesData);
    return {
      categories,
      first: categories[0],
  ...
  computeValueInfo(series, context, categories) {
    ...
    if (this.isStackedVariant(variant)) {
      ...
      // @ts-expect-error: follow up on this
      series.data.map(group => getSeriesData(group.data, categories, categoryProperty, valueProperty))));
    } else {
  ...
  computeCategoryAxisTicks(context, categoryInfo, axisConfig) {
    ...
    const scale = echarts.helper.createScale([categoryInfo.first.valueOf(), categoryInfo.last.valueOf()], model);
    // @ts-expect-error: follow up on this
    const ticks = scale.getTicks(false).map((tick, index) => ({
      ...
      // @ts-expect-error: follow up on this
      ? scale.getFormattedLabel(tick, index, categoryAxisFormatter) : categoryAxisFormatter ? categoryAxisFormatter(scale.getLabel(tick)) : scale.getLabel(tick)),
  ...
  computeValueAxisTicks(context, valueInfo, axisConfig) {
    ...
    // prettier not formatting nested ternaries properly, so turn it off
    // prettier-ignore
    const minValue = axisConfig.min == null ? Math.min(0, valueInfo.minimum) : axisConfig.min === 'dataMin' ? valueInfo.minimum : axisConfig.min;
    // prettier not formatting nested ternaries properly, so turn it off
    // prettier-ignore
    const maxValue = axisConfig.max == null ? valueInfo.maximum : axisConfig.max === 'dataMax' ? valueInfo.maximum : axisConfig.max;
  ...
  generatePlotConfig(series, layout, context, gridIndex) {
    ...
    const valueAxisConfig = {
      ...
      // prettier not formatting nested ternaries properly, so turn it off
      // prettier-ignore
      !isGroupedOrStacked && valueAxisScale === 'shared' ? valueAxisMax && valueAxisMax !== 'dataMax' ? valueAxisMax : data.maxValue : valueAxisMax !== 'dataMaxRoundedUp' ? valueAxisMax : undefined,
      axisLabel: {
        ...(valueAxisFormatter && {
    ...
    const categoryAxisConfig = {
      ...
      axisLabel: {
        ...
        interval: categoryAxisMaxLabelCount && categoryInfo.count > categoryAxisMaxLabelCount ? Math.ceil(categoryInfo.count / categoryAxisMaxLabelCount) - 1 : 0,
        ...(!isHorizontal && {
          overflow: 'break'
        }),
        // margin between the axis label and the axis line
    ...
    const yAxisConfig = {};
    const yAxisInfo = this.computeYAxisInfo(yAxisStyle, isHorizontal ? categoryTicks : valueTicks, isHorizontal);
    layout = this.addAxisPointer(context, layout, yAxisConfig, yAxisInfo, 'y');

    // Configure the X axis
    const xAxisConfig = {};
    const xAxisInfo = this.computeXAxisInfo(args, layout, xAxisStyle, isHorizontal ? valueTicks : categoryTicks, yAxisInfo, isHorizontal);
  ...
  addAxisPointer(context, layout, config, axisInfo, axis) {
    ...
    if (type === 'line') {
      config.axisPointer.lineStyle = {
        ...
        width: axis === 'x' ? pointerStyle.borderLeftWidth || pointerStyle.borderRightWidth : pointerStyle.borderTopWidth || pointerStyle.borderBottomWidth,
        type: axis === 'x' ? pointerStyle.borderLeftStyle || pointerStyle.borderRightStyle : pointerStyle.borderTopStyle || pointerStyle.borderBottomStyle,
    ...
    const labelSize = axis === 'x' ? axisInfo.height + labelStyle.paddingTop + labelStyle.paddingBottom + labelStyle.borderTopWidth + labelStyle.borderBottomWidth : axisInfo.width + labelStyle.paddingLeft + labelStyle.paddingRight + labelStyle.borderLeftWidth + labelStyle.borderRightWidth;
    const labelMargins = axis === 'x' ? labelStyle.marginTop + labelStyle.marginBottom : labelStyle.marginLeft + labelStyle.marginRight;
  ...
  generateTextOverlayConfig(series, args, layout, style) {
    ...
    return (!series.data || series.data.length == 0) && noDataText ? this.generateTextConfig(noDataText,
    // @ts-expect-error: follow up on this
    {
  ...
  computeYAxisInfo(style, ticks, isHorizontal) {
    ...
    // @ts-expect-error: follow up on this
    renderedTicks.map(tick => tick.label), style);
  ...
  computeXAxisInfo(args, layout, style, ticks, yAxisInfo, isHorizontal) {
    ...
    const width = layout.innerWidth - yAxisInfo.width - layout.borderLeftWidth - layout.borderRightWidth;
    ...
    const labelMetrics = computeMaxTextMetrics(
    // @ts-expect-error: follow up on this
    ticks.map(tick => tick.label), style, maxLabelWidth);
    ...
    const widthOverflow = !isHorizontal && categoryAxisType !== 'time' ? 0 : Math.max(0, lastLabelWidth / 2 - (width - lastTick.position * width));
    return {
      width,
      height,
      labelMetrics,
...
export { BarChartModifier as default };
